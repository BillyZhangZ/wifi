...
#ifndef GENRE_SIZE
#define GENRE_SIZE MAX_PTP_STRING_CHARS_GENRE
#define GENRE_SIZE_BYTES (GENRE_SIZE * 2)
#endif
...
typedef struct {
    ...
    uint8_t song_title[TITLE_SIZE];    /*!< Title of Song */
    uint8_t song_artist[ARTIST_SIZE];  /*!< Artist of Song */
    uint8_t song_album[ALBUM_SIZE];    /*!< Album of Song */
    uint8_t song_genre[GENRE_SIZE];    /*!< ptr to buffer to store genre string */
    uint8_t song_year[YEAR_SIZE];      /*!< ptr to buffer to store album year, in
...
extern int32_t codec_extract_metadata_flac(file_meta_data_t *meta_data,
                                        CCI_Ctx *Ctx);
extern int32_t codec_extract_metadata_wma(file_meta_data_t *meta_data,
                                        CCI_Ctx *Ctx);
extern int32_t codec_extract_metadata_ogg(file_meta_data_t *meta_data,
                                        CCI_Ctx *Ctx);
