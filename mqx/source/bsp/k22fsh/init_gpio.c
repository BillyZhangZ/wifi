...
)
{
    ...
    {
        case 0:
            /* Configure GPIOD 0~3 for DSPI0 peripheral function */
            pctl = (PORT_MemMapPtr)PORTD_BASE_PTR;
...
#endif
    /**********************************************/
    /* Below io init from USB V2 TWRK22F51212 BSP */
    /**********************************************/
        /* USB Clock Gating */
        ...
        /*!TODO USB PWR pin need set here! */
...
)
{
    if (dev_if->DEV_INIT_PARAM == &_khci0_dev_init_param)
    {
        ...
        //mux_74hc595_set_bit(BSP_74HC595_0,  BSP_74HC595_GPIO_HOST_EN);
        _bsp_usb_io_init();
    }
    else
    {
...
)
{
    ...
    mux_74hc595_clear_bit(BSP_74HC595_0,  BSP_74HC595_5V_PWREN);
    _time_delay(200);
...
)
{
    switch(dev_num)
    {
        case 0:
            ...
            /* RX Bit and Frame clock mux to other function */
            /* GPIO for RX SSI0_BLCK */
            //PORTC_PCR6 |= PORT_PCR_MUX(0x04);                   /* Configure Port for RX Bit Clock */
            /* GPIO for RX SSI0_FS */
            //PORTC_PCR7 |= PORT_PCR_MUX(0x04);                   /* Configure port for RX Frame Sync*/
