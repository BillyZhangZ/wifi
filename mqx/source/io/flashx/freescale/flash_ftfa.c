...
static void ftfa_ram_function(volatile uint8_t *); 
...
#if BSP_TWR_K22F120M128R || BSP_TWR_K22F120M256R || BSP_LMQ1 || BSP_K22FSH/* K22FN 128/256 has only one block */
...
)
{
    ...
    if((SMC_PMSTAT & SMC_PMSTAT_PMSTAT_MASK) == 0x80) {
        hsrunmode = 1;
        Cpu_SetClockConfiguration(BSP_CLOCK_CONFIGURATION_0);
        Cpu_SetOperationMode(DOM_RUN, 0, 0);
    }

    /* run command and wait for it to finish (must execute from RAM) */ 
    RunInRAM(&ftfa_ptr->FSTAT);
...
*/
    ...
    if(hsrunmode == 1){
        Cpu_SetOperationMode(DOM_HSRUN, 0, 0);
        Cpu_SetClockConfiguration(BSP_CLOCK_CONFIGURATION_3);
    }
