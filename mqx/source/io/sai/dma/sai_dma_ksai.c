...
** experimentally tested default values.
*/
#if (PSP_CPU_MK22FN512 | PSP_CPU_MK22FN256)
...
{   /* Body */
    ...
    if (ksai_info_ptr->CLOCK_SOURCE != I2S_CLK_EXT)
    {
...
)
{
    ...
    if (stats->IN_BUFFER == stats->OUT_BUFFER)
    {
        ...
        i2s_ptr->TCSR &= ~I2S_TCSR_FEIE_MASK;
#if 0
        i2s_ptr->TCSR &= ~(I2S_TCSR_TE_MASK | I2S_TCSR_BCE_MASK);

        while(i2s_ptr->TCSR & I2S_TCSR_BCE_MASK);
#endif
        //printf("%s\n", stats->PACKETS_QUEUED == stats->PACKETS_PROCESSED ?  "U" : "O");
...
)
{
    ...
    switch (cmd)
    {
        ...
        case IO_IOCTL_I2S_UPDATE_TX_STATUS:
            ...
            if(io_info_ptr->FIRST_IO && (io_info_ptr->DMA_KICKOFF == TRUE) &&
               (stats->IN_BUFFER >= buffer->START_PERIOD + (buffer->PERIODS / 2) * buffer->SIZE))

            {
                ...
                i2s_ptr->TCR4 |= (I2S_TCR4_FCONT_MASK);
...
{    
    ...
    */
    ...
    if(clk_source != I2S_CLK_EXT){
        i2s_ptr->MCR &= ~I2S_MCR_MICS_MASK;
        i2s_ptr->MCR |= I2S_MCR_MICS(clk_source);
    }
...
)
{
    ...
    bool found = FALSE;
    uint32_t source_freq = 0;
    //uint32_t origfrequency = frequency;
    ...
    */
    for (fract = 1; fract < FRACT_MAX; fract++)
    {
       ...
       {
           iter_mclk = (uint32_t)((uint64_t)source_freq * fract / divide );
    ...
    *div = divide;
    ...
    //printf("source %d, fract %d, divide %d, orig dest freq %d, current dest freq %d   found %d \n", source_freq, fract, divide, origfrequency,  frequency, found);
