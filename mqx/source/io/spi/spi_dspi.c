   ...
   )
{
    ...
    else if((params->ATTR & SPI_ATTR_TRANSFER_MODE_MASK) == SPI_ATTR_SLAVE_MODE)
    {
#if 0
//		//hal init
//		dspi_ptr->MCR &= (~DSPI_MCR_MDIS_MASK);
//		dspi_ptr->MCR |= DSPI_MCR_HALT_MASK;
//		dspi_ptr->SR |= (DSPI_SR_TCF_MASK | DSPI_SR_EOQF_MASK | DSPI_SR_TFUF_MASK|\
//						DSPI_SR_TFFF_MASK | DSPI_SR_RFOF_MASK | DSPI_SR_RFDF_MASK);
//		dspi_ptr->TCR = 0;
//		dspi_ptr->CTAR[0] = 0x78000000;
//		dspi_ptr->CTAR[1] = 0x78000000;
//		dspi_ptr->RSER = 0;
//        
//        dspi_ptr->MCR &= (~DSPI_MCR_ROOE_MASK);
//        dspi_ptr->MCR &= (~DSPI_MCR_PCSIS_MASK);
//		
//		dspi_ptr->PUSHR = 0;
//		dspi_ptr->MCR |= DSPI_MCR_CLR_RXF_MASK;
//		dspi_ptr->MCR |= DSPI_MCR_CLR_TXF_MASK;
//		dspi_ptr->MCR |=(DSPI_MCR_MDIS_MASK | DSPI_MCR_HALT_MASK);
//		
//		/*set slave mode*/
//		dspi_ptr->MCR &= (~DSPI_MCR_MSTR_MASK);
//		dspi_ptr->CTAR[0] = 0x38000000;
//		dspi_ptr->MCR &= (~DSPI_MCR_MDIS_MASK);
...
//		dspi_ptr->MCR &= (~DSPI_MCR_HALT_MASK);
		...
		dspi_ptr->MCR |= DSPI_MCR_HALT_MASK;		
		/*clear FIFOs*/
		dspi_ptr->MCR |= (DSPI_MCR_CLR_RXF_MASK | DSPI_MCR_CLR_TXF_MASK);		
		/*w1c SR status bits*/
		dspi_ptr->SR |= (DSPI_SR_TCF_MASK | DSPI_SR_EOQF_MASK | DSPI_SR_TFUF_MASK|\
						DSPI_SR_TFFF_MASK | DSPI_SR_RFOF_MASK | DSPI_SR_RFDF_MASK);
		
		/*Clear TCR */
		dspi_ptr->TCR = 0x00000000;
		dspi_ptr->RSER = 0x00000000;		
		
        /*disable IRQs*/
        dspi_ptr->RSER = (DSPI_RSER_RFDF_RE_MASK |DSPI_RSER_TFFF_RE_MASK|\
		DSPI_RSER_RFOF_RE_MASK|DSPI_RSER_TFUF_RE_MASK);
		...
		/* Set slave mode */
		dspi_ptr->MCR &= (~DSPI_MCR_MSTR_MASK);
	   /*set ctar parameters, FRMZ = 7; CPOL = 0; CPHA = 0*/
		//dspi_ptr->CTAR[0] = 0x38000000;
		ctar = 0;
        ...
        dspi_ptr->CTAR[0] = ctar;
		/*disable PCSIS, enable MDIS, enable HALT, start the transfer*/
		dspi_ptr->MCR &= (~DSPI_MCR_PCSIS_MASK);
		dspi_ptr->MCR &= (~DSPI_MCR_MDIS_MASK);
		/*enable SPI*/
		dspi_ptr->MCR &= (~DSPI_MCR_HALT_MASK);
    ...
    )
{
	...
	if((dspi_ptr->MCR & DSPI_MCR_MSTR_MASK) != 0)
      {
