    ...
    )
{
    ...
    if (dev_data->BUS_OWNER)
    {
        if (!cs_keep)
        {
            /*deassert CS pin only in master mode*/
            if((dev_data->PARAMS.ATTR & SPI_ATTR_TRANSFER_MODE_MASK) == SPI_ATTR_MASTER_MODE)
            {
    ...
    )
{
    ...
    switch (cmd) {
		...
		case IO_IOCTL_SPI_GET_ATTRIBUTES:
            if (NULL == param_ptr)
            {
                result = SPI_ERROR_INVALID_PARAMETER;
            }
            ...
            {
                *((uint32_t *)param_ptr) = dev_data->PARAMS.ATTR;
            }
            break;

        case IO_IOCTL_SPI_SET_ATTRIBUTES:
            if (NULL == param_ptr)
            {
                result = SPI_ERROR_INVALID_PARAMETER;
            }
            ...
            {
                dev_data->PARAMS.ATTR = *((uint32_t *)param_ptr);
                dev_data->PARAMS_DIRTY = TRUE;
            }
            break;
