...
{ /* Body */
      ...
      int_io_dev_ptr->TX_DEV_PUTC = TRUE;      
      ...
      sci_info_ptr->TX_BUF = _mem_alloc_system_zero_uncached(sci_init_ptr->OQUEUE_SIZE);
    ...
    dma_channel_setup(sci_info_ptr->RX_DCH, 2, DMA_CHANNEL_FLAG_LOOP_MODE);
    ...
    _time_init_ticks(p_ticks, 1);  //Ethan: 1 tick is 5ms. default is 20 tick.
...
static void _kuart_period_isr(void * data_ptr)
{
    ...
    else
    {
        ...
        /* transfered num is "dma req size" - "dma req remaining" */
        if (sci_info_ptr->RX_ACTIVE == sci_info_ptr->RX_BUF + sci_info_ptr->RX_DMA_SEQ * (int_io_dev_ptr->OUT_QUEUE->MAX_SIZE / 2))
	{ /* Make sure no DMA finish event happen */
            /* transfered num is "dma req size" - "dma req remaining" */
            num = int_io_dev_ptr->OUT_QUEUE->MAX_SIZE / 2 - num;
		
            for (i=sci_info_ptr->RX_BYTES; i<num; i++)
            {
                cc = sci_info_ptr->RX_ACTIVE[i];
                _io_serial_int_addc(int_io_dev_ptr, cc);
            }

            sci_info_ptr->RX_CHARS += num - sci_info_ptr->RX_BYTES;
            sci_info_ptr->RX_BYTES = num;
            sci_info_ptr->RX_KEEP = 1;
	}
...
static void _kuart_prepare_tx_dma(IO_SERIAL_INT_DEVICE_STRUCT_PTR int_io_dev_ptr,uint32_t size)
{
    ...
    /* force the higher driver to wake up output task */
    _io_serial_int_nextc(int_io_dev_ptr);
...
static void _kuart_prepare_rx_dma(IO_SERIAL_INT_DEVICE_STRUCT_PTR int_io_dev_ptr)
{
    ...
    sci_info_ptr = int_io_dev_ptr->DEV_INFO_PTR;
    sci_ptr = sci_info_ptr->SCI_PTR;
    dma_tcd_reg2mem(&tcd, &(sci_ptr->D) , 1, sci_info_ptr->RX_BUF, int_io_dev_ptr->IN_QUEUE->MAX_SIZE / 2);
    dma_transfer_submit(sci_info_ptr->RX_DCH, &tcd, &sci_info_ptr->RX_DMA_SEQ);
    dma_tcd_reg2mem(&tcd, &(sci_ptr->D) , 1, sci_info_ptr->RX_BUF + int_io_dev_ptr->IN_QUEUE->MAX_SIZE / 2, int_io_dev_ptr->IN_QUEUE->MAX_SIZE / 2);
    dma_transfer_submit(sci_info_ptr->RX_DCH, &tcd, &sci_info_ptr->RX_DMA_SEQ);
