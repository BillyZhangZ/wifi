...
typedef struct kuart_info_struct
{
   ...
   void       (_CODE_PTR_ OLD_ISR_ERR)(void *);
   void       (_CODE_PTR_ OLD_ISR_TXRX)(void *);
   void       (_CODE_PTR_ OLD_ISR_EXCEPTION_HANDLER)(uint32_t, uint32_t, void *,
               void *);
   void                             *OLD_ISR_DATA;
   void                             *OLD_ISR_ERR_DATA;
   void                             *OLD_ISR_TXRX_DATA;
   ...
   uint8_t *RX_BUF;
   uint8_t *TX_BUF;
   uint8_t *RX_ACTIVE;            /* the active RX buffer pointer for pingpong */   
   uint16_t RX_BYTES;   
   uint8_t TX_KEEP;
   uint8_t RX_KEEP;
...
extern uint32_t _kuart_int_install(char *, KUART_INIT_STRUCT_CPTR, uint32_t, uint32_t);
extern void   *_bsp_get_serial_base_address(uint8_t);
extern uint32_t _kuart_dma_install(char *, KUART_INIT_STRUCT_CPTR, uint32_t, uint32_t);
extern uint32_t _kuart_mix_install(char *, KUART_INIT_STRUCT_CPTR, uint32_t, uint32_t);
