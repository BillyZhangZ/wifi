...
A_UINT32        dns_block_time=(DHCP_WAIT_TIME *2);
...
A_STATUS custom_socket_context_init()
{
    ...
    {	
      if((pcustctxt = A_MALLOC(sizeof(SOCKET_CUSTOM_CONTEXT),MALLOC_ID_CONTEXT)) == NULL){
        ...
        _lwevent_create(&pcustctxt->sockRxWakeEvent, 0/* no auto clear */); 		
        _lwevent_create(&pcustctxt->sockTxWakeEvent, 0/* no auto clear */); 		
...
#endif	          		
	    _lwevent_destroy(&pcustctxt->sockRxWakeEvent); 
            _lwevent_destroy(&pcustctxt->sockTxWakeEvent); 
...
A_STATUS blockForResponse(A_VOID* pCxt, A_VOID* ctxt, A_UINT32 msec, A_UINT8 direction)
{
    ...
    if(direction == RX_DIRECTION)
    {
        if(pcustctxt->respAvailable == A_FALSE)
        {
            ...
            if(MQX_OK != _lwevent_wait_ticks(&pcustctxt->sockRxWakeEvent, 0x01, FALSE, ret)){  
                result = A_ERROR;	                                               
            }else{                                                      
                _lwevent_clear(&pcustctxt->sockRxWakeEvent, 0x01);                            
            }     
    ...
    else if(direction == TX_DIRECTION)
    {
        if(pcustctxt->txUnblocked == A_FALSE)
        {
            ...
            if(MQX_OK != _lwevent_wait_ticks(&pcustctxt->sockTxWakeEvent, 0x01, FALSE, ret)){  
                result = A_ERROR;	                                               
            }else{                                                      
                _lwevent_clear(&pcustctxt->sockTxWakeEvent, 0x01);                            
            }     
	 ...
	 if(direction == RX_DIRECTION)
        {
            ...
            {                    
               _lwevent_set(&pcustctxt->sockRxWakeEvent, 0x01);
            }
        }
        else if(direction == TX_DIRECTION)
        {
            ...
            {                    
               _lwevent_set(&pcustctxt->sockTxWakeEvent, 0x01);
            }
        }
	return result;	
}
...
A_INT32 custom_http_set_post_cb(void *handle, void* cxt, void* callback)
{
    A_VOID *pCxt=((ENET_CONTEXT_STRUCT_PTR)handle)->MAC_CONTEXT_PTR;
...
* RETURNS: socket handle or A_ERROR 
*****************************************************************************/
A_INT32 custom_httpc_method(void* handle, A_UINT32 command, A_UINT8* url, A_UINT8 *data, A_UINT8 **output)
{
#if 0 /* keviny */
		A_VOID *pCxt = handle;
#else
...
A_INT32  custom_ip_set_block_time(void *handle , A_INT32 blockSec)
{
		if (blockSec<0)
		{
				return A_ERROR;
		}
		dns_block_time = blockSec*1000;
		return A_OK;
}
...
A_INT32 custom_ota_upgrade(void *handle,A_UINT32 addr,A_CHAR *filename,A_UINT8 mode,A_UINT8 preserve_last,A_UINT8 protocol,A_UINT32 *resp_code,A_UINT32 *ret_len )
{
    A_VOID *pCxt=((ENET_CONTEXT_STRUCT_PTR)handle)->MAC_CONTEXT_PTR;
...
A_INT32 custom_ota_read_area(void *handle,A_UINT32 offset,A_UINT32 size,A_UCHAR *buffer,A_UINT32 *ret_len)
{
    A_VOID *pCxt=((ENET_CONTEXT_STRUCT_PTR)handle)->MAC_CONTEXT_PTR;
