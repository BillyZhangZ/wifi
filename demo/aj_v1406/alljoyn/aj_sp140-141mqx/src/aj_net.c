...
AJ_Status AJ_Net_Connect(AJ_NetSocket* netSock, uint16_t port, uint8_t addrType, const uint32_t* address)
{
    ...
    if (ret < 0) {
        ...
        status = ConfigNetSock(netSock, AJ_Net_Recv, 512, AJ_Net_Send, 1500);
        if (status == AJ_OK) {
...
static int MCastUp6()
{
    ...
    memcpy(group6.ipv6mr_interface.addr, gblAddr, sizeof(gblAddr));
...
AJ_Status AJ_Net_MCastUp(AJ_NetSocket* netSock)
{
    ...
    if ((udpSock != A_SOCK_INVALID) || (udp6Sock != A_SOCK_INVALID)) {
        status = ConfigNetSock(netSock, RecvFrom, 0, SendTo, 256);
        if (status == AJ_OK) {
