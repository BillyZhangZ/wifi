...
A_STATUS ath_set_mode(A_UINT32 device,char* mode)
{
    ...
    else
    {
        ...
        if (dev_status == A_FALSE)
        {
            return A_ERROR;
        }
        
        error = ENET_mediactl (enethandle,ENET_SET_MEDIACTL_MODE,&inout_param);
        if (ENET_OK != error)
        {
            return (A_STATUS) error;
        }
    ...
    return (A_STATUS) ENETERR_INVALID_DEVICE;
}
...
A_STATUS ath_get_essid(A_UINT32 device, char*  essid){
  return((A_STATUS) iwcfg_get_essid(device,essid));
}
...
A_INT32 NetConnect(ATH_NETPARAMS *pNetparams)
{
	...
	do{
		...
		{	
	    	...
	    	ATH_SET_ESSID (get_active_device(), (char*)pNetparams->ssid);
			...
			if(pNetparams->sec_type == ENET_MEDIACTL_SECURITY_TYPE_WPA2){
		        ...
		        if(A_OK != ENET_mediactl (get_active_device_handle(),ENET_MEDIACTL_VENDOR_SPECIFIC,&ath_param)){
		        	break;
		        }
		        strcpy(ver,"wpa2");
		        ATH_SET_SEC_TYPE (get_active_device(),ver); 
				
		        if(strlen((char*)pNetparams->u.passphrase) != 64)
				{								
					ATH_SET_PASSPHRASE (get_active_device(),(char*)pNetparams->u.passphrase);
				}
				else
				{
				...
				ATH_SET_SEC_TYPE (get_active_device(),"wep");
				set_wep_keys (get_active_device(),(char*)pNetparams->u.wepkey,
...
{	
   ...
   /*Check if driver is loaded*/
    ...
    error = ENET_mediactl (enethandle,ENET_GET_MEDIACTL_POWER,&inout_param);
    if (A_OK != error)
    {
        return error;
    }
...
A_INT32 wmi_set_scan(A_UINT32 dev_num)
{
    ...
    if (dev_num < BSP_ENET_DEVICE_COUNT)
    {
        ...
        error = ENET_mediactl (enethandle,ENET_SET_MEDIACTL_SCAN,NULL);
        if (A_OK != error)
        {
            return error;
        }

        error = ENET_mediactl (enethandle,ENET_GET_MEDIACTL_SCAN,&param);
        if (A_OK != error)
        {
            return error;
        }
...
A_INT32 wmi_ping(A_INT32 argc, char* argv[] ){
    ...
    for(index = 1; index < argc ; index++)
    {
        ...
        {
            error= ath_inet_aton(argv[index], &hostaddr);
            if (error != -1)
            {
