...
BleStatus SYSTEM_SetPersistentInformation(BD_ADDR *addr, U8 infoType, U8* infoValue, U8 InfoLen){
	...
	SYSTEM_Log("[SYSTEM] Try to SET infotype [%d] information in persistent memory\r\n", infoType);  
...
#if (BLE_PRINT_DEBUG_TRACES == 1)
	SYSTEM_Log("[SYSTEM] Try to GET infotype [%d] information in persistent memory\r\n", infoType);  
#endif //(BLE_PRINT_DEBUG_TRACES == 1)
...
BleStatus SYSTEM_RemovePersistentInformation(BD_ADDR *addr, U8 infoTypeFrom, U8 infoTypeTo){
	...
	#if 1
#if (BLE_PRINT_DEBUG_TRACES == 1)
	SYSTEM_Log("[SYSTEM] Try to REMOVE infotype [%d-%d] information in persistent memory\r\n", 
	    infoTypeFrom, 
	    infoTypeTo);  
#endif //(BLE_PRINT_DEBUG_TRACES == 1)
